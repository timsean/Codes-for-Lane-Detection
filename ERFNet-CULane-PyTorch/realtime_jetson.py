...
input_example = torch.ones((1, 3, TRAIN_IMG_H, TRAIN_IMG_W)).float().cuda()
print(input_example.size())
model_trt = torch2trt(model, [input_example], log_level=trt.Logger.WARNING)
#torch.save(model_trt.state_dict(), 'erfnet_trt.pth')
#model_trt = TRTModule()
#model_trt.load_state_dict(torch.load('erfnet_trt.pth'))
